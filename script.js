...
async function deleteLeagueFixtures(comp){
 ...
 const batch=db.batch(); fixtures.forEach(f=>batch.delete(db.collection('fixtures').doc(f.id)));
 await batch.commit(); await loadData(); alert(`${comp} fixtures deleted successfully.`); adminTab('fixtures');
}
